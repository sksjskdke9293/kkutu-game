...
const MORAE_DICTIONARIES = [ "basic", "standard", "complex" ];
...
exports.Data = function(data){
	...
	// Ranked results belong to signed-in accounts.  Guests intentionally keep
	// theirs in the browser so they can play without an account.
	this.ranked = data.ranked ? {
		rating: Math.max(0, Number(data.ranked.rating) || 0),
...
exports.Room = function(room, channel){
	...
	my.id = room.id || _rid;
	my.channel = channel;
	my._autoMatch = !!room._autoMatch;
	// Ranked is intentionally set only by the master-side ranked matchmaker.
	// It is exported so every client can use the ranked presentation and music.
	my.ranked = !!room.ranked;
	my.opts = {};
	/*my.title = room.title;
	...
	my.come = function(client){
		...
		if(Cluster.isWorker){
			...
			my.export(client.id);
			if(my._autoMatch && my.players.length === 2) {
				my._autoMatch = false;
				setTimeout(function(){
					if(ROOM[my.id] !== my || my.gaming || my.players.length !== 2) return;
					my.players.forEach(function(id){ if(DIC[id]) DIC[id].ready = true; });
					my.ready();
				}, 1500);
			}
		}
	...
	my.set = function(room){
		...
		if(room.opts && my.opts){
			...
			dictionary = String(room.opts.dictionary || "standard");
			my.opts.dictionary = MORAE_DICTIONARIES.includes(dictionary) ? dictionary : "standard";
			my.opts.shield = room.opts.shield == null ? 15 : Math.max(0, Math.min(100, Math.floor(Number(room.opts.shield)) || 0));
	...
	my.roundEnd = function(data){
		...
		for(i in res){
			...
			if(my.ranked && !o.guest){
				...
				if(!isFinite(Number(o.data.ranked.rating))) o.data.ranked.rating = 1000;
				o.data.ranked.rating = Math.max(0, Number(o.data.ranked.rating) + (res[i].rank === 0 ? 25 : -15));
